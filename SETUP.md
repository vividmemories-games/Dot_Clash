...
**New Mac?** Back up the keystore and passwords first — see [MIGRATION_RUNBOOK.md](../MIGRATION_RUNBOOK.md).
...
**Canonical diagrams (control + data flow):** [`docs/architecture.md`](docs/architecture.md) — local vs Challenge game paths, Firebase callables, navigation, CI gates.

**Two game backends:**

| Mode | Authority | Where |
|------|-----------|--------|
| Campaign, Quick Match, Local, Daily | Client `GameRules` on device | `lib/features/game/` |
| Challenge a Friend (live 1v1, 6×6) | Server `functions/src/game_rules.ts` + callables | `lib/features/challenge/` + `challenges/{code}` |

Challenge settlement is **history-only** (`recordChallengeMatch`); campaign economy still uses `completeCampaignLevel` / `settleQuickMatch`. See [summary.md](docs/summary.md) for implementation notes.

```
lib/
├── main.dart                  Firebase, App Check, FCM background, ProviderScope
├── app.dart                   MaterialApp.router, ads + FCM init, deep-link ingress
├── firebase_options.dart      Generated by flutterfire configure
├── core/
│   ├── env/app_env.dart       Flavors, ad unit IDs, OAuth client IDs, App Links base URL
│   ├── router/app_router.dart GoRouter — /challenge/*, auth redirects, ?next= preservation
│   ├── deep_links/            challenge_link_parser.dart
│   └── theme/                 AppColors, AppTextStyles, AppTheme
├── shared/
│   ├── layout/                AppSpacing, ResponsiveLayout
│   └── widgets/               AppShell, NeonCard, NeonButton, ...
├── features/
│   ├── auth/                  Google, Apple, anonymous sign-in
│   ├── onboarding/            First-launch splash + coach tour hooks
│   ├── home/                  Play modes grid → Challenge hub tile
│   ├── game/                  Client-authoritative board (Campaign, Quick, Local, Daily)
│   │   ├── domain/            GameState, GameRules (pure Dart)
│   │   ├── providers/         GameNotifier, turn timer
│   │   └── presentation/      GameScreen (shared UI; challenge branches gated)
│   ├── challenge/             Live 1v1 — lobby, hub, rivals, ChallengeGameNotifier
│   ├── campaign/              Worlds JSON, map, daily puzzle
│   ├── profile/               Firestore/mock repo, progression, match history
│   ├── shop/                  IAP + coin shop
│   ├── settings/              Theme, consent, account deletion entry
│   ├── ai/                    AiPlayer presets
│   ├── powerups/              Boost catalog + consumption
│   ├── account/               deleteUserData callable wrapper
│   └── contact/               Support mailto screen
├── services/
│   ├── ads/                     AdMob, UMP consent, rewarded routing
│   ├── iap/                     StoreKit / Play Billing
│   ├── analytics/               Firebase Analytics (+ challenge_started/finished)
│   ├── backend/                 CallableBackend HTTPS wrapper
│   ├── firebase/                App Check
│   ├── push/                    FCM token registration + invite tap routing
│   └── deep_links/              ChallengeIngressListener (HTTPS + custom scheme)
└── features/multiplayer/      Empty scaffold — not used; live PvP is challenge/

functions/src/
├── game_rules.ts              Server GameRules (Dart parity)
├── challenge.ts               create/join/move/abandon/record callables
├── challenge_scheduler.ts     processChallengeTimeouts (scheduled)
├── notifications.ts           registerFcmToken, invite push
├── economy.ts / lives.ts      Shop, quick match, lives
└── index.ts                   Callable exports

firestore.rules                profiles, matches, challenges (client read-only on rooms)
firestore.indexes.json         Challenge scheduler composite indexes
.github/workflows/dart.yml     flutter analyze --no-fatal-infos, flutter test
```
